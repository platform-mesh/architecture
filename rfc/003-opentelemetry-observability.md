...
Every environment deploys two `OpenTelemetryCollector` CRs: a gateway and a DaemonSet log agent. The configuration is identical — the only difference between local and production is the gateway replica count.

### Local Development

```yaml
# Gateway — single replica, same config as production
...
spec:
  ...
  targetAllocator:
    ...
    prometheusCR:
      enabled: true
  config:
    # ... gateway pipeline config from above

---
# DaemonSet log agent — identical across environments
apiVersion: opentelemetry.io/v1beta1
kind: OpenTelemetryCollector
metadata:
  name: platform-mesh-logagent
  namespace: observability
spec:
  mode: daemonset
  volumeMounts:
  ...
  volumes:
    - name: varlogpods
      hostPath:
        path: /var/log/pods
  config:
    # ... DaemonSet log agent config from above
```

### Production

```yaml
# Gateway — only replicas differ
apiVersion: opentelemetry.io/v1beta1
kind: OpenTelemetryCollector
metadata:
  name: platform-mesh-gateway
...
spec:
  mode: statefulset
  replicas: 3  # ← the only difference
  targetAllocator:
    enabled: true
    allocationStrategy: consistent-hashing
    prometheusCR:
      enabled: true
  config:
    # ... identical gateway pipeline config

---
# DaemonSet log agent — identical to local
apiVersion: opentelemetry.io/v1beta1
kind: OpenTelemetryCollector
metadata:
  name: platform-mesh-logagent
